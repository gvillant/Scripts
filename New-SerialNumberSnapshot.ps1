$SerialNumberCSVFile = "C:\vms\Dropship_VMs.csv"
...
function Invoke-MenuVM
{
#Build VM Menu
write-host "Choose VM to create a new SerialNumber snapshot" -ForegroundColor Green 
write-host "VM should be OFF" -ForegroundColor Green 

$VMs = Get-VM
$menuVM = @{}
for ($i=1;$i -le $VMs.count; $i++) 
{ Write-Host "$i. $($VMs[$i-1].name),$($VMs[$i-1].State)" 
$menuVM.Add($i,($VMs[$i-1].name))}

[int]$VM = Read-Host 'Enter selection'
$Global:VMselection = $menuVM.Item($VM)
}

function Invoke-BulkMenu
{
#Build Bulk Menu
write-host "Bulk or unique serial ?" -ForegroundColor Green 
write-host "1. Bulk"
write-host "2. Serial list"
[int]$Global:Bulk = Read-Host 'Enter selection'
}

function Invoke-MenuSerial
{
#Build SerialNumber Menu
write-host "Choose SerialNumber " -ForegroundColor Green 
...
#Snapshot
$VMToSnap = Get-VM $VMselection
$Checkpoint = Checkpoint-VM -VM $VMToSnap -SnapshotName $SnapshotName -Passthru
While (-not (@(Get-VMSnapshot $VMToSnap | select -ExpandProperty Id)).Contains($Checkpoint.Id)) 
    {
        Write-Host "Wait for checkpoint to be in list"
        Start-Sleep -Seconds 1
    }

#Restore parent snapshot
$NewCheckpoint = Get-VMSnapshot -Name $SnapshotName -VMName $VMselection
$ParentSnapshot = Get-VMSnapshot -ParentOf $NewCheckpoint
Restore-VMSnapshot -VMSnapshot $ParentSnapshot -Confirm:$false
}
